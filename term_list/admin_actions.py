...
def export_chosen_concept_as_csv(
    request: HttpRequest,
    queryset: QuerySet,
    ...
    for row_idx, obj in enumerate(queryset, start=1):
        for col_index, field in enumerate(selected_fields):
            if field.lower() == "synonyms":
                ...
                link = (
                    request.build_absolute_uri(reverse("term_metadata"))
                    + f"?q={obj.pk}"
                )
                worksheet.write_url(
                    row=row_idx, col=col_index, url=link, string=row_data
                )
            elif (field in field_mapping.keys()) and (
                field in ["Senaste ändring", "Datum skapat"]
            ):
                ...
                if hasattr(obj, field.lower()):
                    ...
                    if isinstance(value, BaseManager):
                        row_data = next(iter(value.values_list("name", flat=True)), "")
                    else:
                        row_data = value
                else:
                    matching_attr = next(
                        (
                            ...
                            if attr.attribute.display_name == field
                        ),
                        None,
                    )
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="exporterad_begrepp_{datetime.datetime.now().strftime("%Y_%m_%d-%H:%M:%S")}.xlsx"'
    )
...
def delete_allowed_objects(
    modeladmin, request, queryset, *, object_label="objekt", permission_check=None
):
...
def change_dictionaries(modeladmin, request, queryset):
    ...
    if "apply" in request.POST:
        ...
        if selected_dictionaries:
            ...
            modeladmin.message_user(
                request,
                f"{queryset.count()} Begrepp updated with the selected Dictionaries.",
            )
        return redirect(request.get_full_path())
...
def export_chosen_concepts_action(modeladmin, request, queryset):
    ...
    db_table_attrs = [
        ...
        if field.name
        not in ["concept_fk", "conceptcomment", "conceptexternalfiles", "attributes"]
    ]

    attribute_names = (
        Attribute.objects.filter(attributevalue__term__in=queryset)
    ...
    chosen_concepts = [
        {"pk": i[0], "term": i[1]} for i in queryset.values_list("pk", "term")
    ]

    return render(
        request,
        "choose_export_attrs_intermediate.html",
        context={"db_table_attrs": db_table_attrs, "chosen_concepts": chosen_concepts},
    )
