...
class NewSearch(unittest.TestCase):
    ...
    def test_open_base_page_and_perform_search(self):
        ...
        search_results_table = self.browser.find_element_by_id('searchResultTable')
    ...
    def test_open_beställ_begrepp(self):
        ...
        f = open("term_list/tests/test_fil.txt", "w")
        f.write(lorem.paragraph())
        f.close()

        # attach a file
        attach_file = self.browser.find_element_by_xpath('//*[@id="id_file_field"]')
        attach_file.send_keys(os.path.abspath('./term_list/tests/test_fil.txt'))
